...
from llava.model.blip_encoder.processors.blip_processors import BlipImageBaseProcessor
...
class ModelArguments:
    ...
    out_dim: Optional[int] = field(default=256)
    ce_loss_weight: Optional[float] = field(default=1.0)
    dice_loss_weight: Optional[float] = field(default=0.5)
    bce_loss_weight: Optional[float] = field(default=2.0)
...
def preprocess_alfred(
    ...
    snippet_2_length = len(tokenizer("</Img>").input_ids)
    for img_token_id in img_token_indices:
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, 
        ...
        list_data_dict = json.load(open(data_path, "r")) # load trajectory json 
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        data_dict = {}
        image_files = sources['img_list']
        image_folder = self.data_args.image_folder
        processor = self.data_args.image_processor
        img_list, orig_img_list = [], []
        for image_file in image_files:
            image_file = image_file.replace("png", "jpg")
            task_name, trial_name = sources["task_name"].split("/")
            image = Image.open(os.path.join(image_folder, task_name, trial_name, "raw_images", image_file)).convert('RGB')
            img_arr = np.array(image)[:, :, :3] # remove alpha channel
            orig_img = torch.from_numpy(img_arr).permute(2, 0, 1).float() # [3, 300, 300]
        ...
        data_dict['images'], data_dict['orig_images'] = img_list, orig_img_list
        data_dict['input_ids'], data_dict['labels'] = preprocess_alfred(sources, self.tokenizer)
        obj_positions, obj_bboxes, obj_masks =  [], [], []
        for obj_dict in sources['object_args']:
            obj_positions.append(obj_dict["index"])
            obj_bboxes.append(obj_dict["bbox"])
            obj_masks.append(obj_dict["mask"])
        data_dict["obj_positions"], data_dict["obj_bboxes"], data_dict["obj_masks"] = obj_positions, obj_bboxes, obj_masks
        return data_dict
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor|List[List]]:
        ...
        batch["images"], batch['orig_images'], batch["obj_positions"], batch["obj_bboxes"], batch["obj_masks"] = [], [], [], [], []
        for instance in instances:
            batch["images"].append(instance["images"])
            batch['orig_images'].append(instance['orig_images'])
...
def train():
    ...
    if training_args.lora_enable:
        ...
        lora_target_modules = find_linear_layers(model, ["q_proj", "v_proj"])
