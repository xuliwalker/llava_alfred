...
def preprocess_llama_2(
    ...
    # print("converstaions:  ", conversations)
    ...
    for conversation, target in zip(conversations, targets):
        ...
        for i, rou in enumerate(rounds):
            ...
            if has_image:
                ...
                instruction_len = len(tokenizer_image_token(parts[0], tokenizer)) - 2  # 删除[INST] and [/INST]
            else:
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        # sources keys: task_name, trial_name, goal, instr, img_list, act_img_seq
        # process image files
        data_dict = {}
        ...
        for image_file in image_files:
            ...
            image = Image.open(os.path.join(image_folder, sources["task_name"], sources["trial_name"], "raw_images", image_file)).convert('RGB')
            print("before processing image size:  ", image.size)
            if self.data_args.image_aspect_ratio == 'pad':
                image = expand2square(image, tuple(int(x*255) for x in processor.image_mean))
                image = processor.preprocess(image, return_tensors='pt')['pixel_values'][0]
            else:
                image = processor.preprocess(image, return_tensors='pt')['pixel_values'][0]
            print("after processing image size:  ", image.size)
