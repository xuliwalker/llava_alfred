...
class LlavaMetaModel:
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        self.Qformer, self.query_tokens = blip.init_Qformer(
            num_query_token, self.vision_tower.visual_encoder.num_features
        ...
        blip.load_from_pretrained(self, url_or_filename=q_former_model)
        ...
        self.mm_projector = nn.Linear(self.Qformer.config.hidden_size, self.config.hidden_size) # 768 -> 4096
        
        # pretained_ckpt = "/home/maodongxing/project/LLaVA/checkpoints/projector.pth"
        pretained_weight = torch.load("/home/xuli/dongxing/LLaVA/checkpoints/projector.pth")

        # pretained_weight = torch.load("/home/maodongxing/project/LLaVA/checkpoints/projector.pth")
        self.mm_projector.load_state_dict(pretained_weight)
...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images):
        ...
        image_embeds = self.get_model().get_vision_tower()(images).to(device)
        image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(device)
        query_tokens = self.get_model().get_query_token()
        query_tokens = query_tokens.expand(image_embeds.shape[0], -1, -1)
        Qformer = self.get_model().get_Q_former()
        ...
        image_features = self.get_model().mm_projector(query_output.last_hidden_state)
        return image_features
    
    def prepare_inputs_labels_for_alfred(self, input_ids, attention_mask, past_key_values, labels, images):
        new_input_embeds = []
        new_labels = []
